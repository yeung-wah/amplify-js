...
var RestClient = class {
  ...
  async ajax(urlOrApiInfo, method, init) {
    logger.debug(method, urlOrApiInfo);
    let parsed_url;
    let url;
    let region = "us-east-1";
    let service = "execute-api";
    let custom_header = void 0;
    ...
    const custom_header_obj = typeof custom_header === "function" ? await custom_header() : void 0;
    params.headers = {
      ...libraryHeaders,
      ...custom_header_obj,
      ...initParams.headers
    };
    const { search, ...parsedUrl } = (0, import_url.parse)(url, true, true);
    params.url = (0, import_url.format)({
      ...parsedUrl,
      query: {
        ...parsedUrl.query,
    ...
    return this.Credentials.get().then((credentials) => {
      return this._signed({ ...params }, credentials, isAllResponse, {
        ...
        if (DateUtils.isClockSkewError(error)) {
          ...
          const requestDate = DateUtils.getDateFromHeaderString(params.headers["x-amz-date"]);
  ...
  updateRequestToBeCancellable(promise, cancelTokenSource) {
  ...
  _signed(params, credentials, isAllResponse, { service, region }) {
    const {
      signerServiceInfo: signerServiceInfoParams,
      ...otherParams
    } = params;
    const endpoint_region = region || this._region || this._options.region;
    const endpoint_service = service || this._service || this._options.service;
    const creds = {
    ...
    return (0, import_axios.default)(signed_params).then((response) => isAllResponse ? response : response.data).catch((error) => {
      logger.debug(error);
      throw error;
    });
  }
  _request(params, isAllResponse = false) {
    return (0, import_axios.default)(params).then((response) => isAllResponse ? response : response.data).catch((error) => {
      logger.debug(error);
      throw error;
    });
  }
...
var logger2 = new Logger2("RestAPI");
var RestAPIClass = class {
  constructor(options) {
    this._api = null;
    this.Credentials = Credentials2;
    this._options = options;
    logger2.debug("API Options", this._options);
  }
  getModuleName() {
    return "RestAPI";
  }
  configure(options) {
    ...
    logger2.debug("configure Rest API", { opt });
    ...
    if (Array.isArray(opt.endpoints)) {
      opt.endpoints.forEach((endpoint) => {
        if (typeof endpoint.custom_header !== "undefined" && typeof endpoint.custom_header !== "function") {
          logger2.warn("Rest API " + endpoint.name + ", custom_header should be a function");
  ...
  createInstance() {
    logger2.debug("create Rest API instance");
